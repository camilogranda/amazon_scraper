...
def get_date_place(param):
    ...
    if result:
        ...
        place = param.replace(date_part, '').replace(
            'Reviewed in', '').replace('on', '').strip()
...
class ReviewsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        for asin in asin_list:
            ...
            yield scrapy.Request(reviews_base_url.format(asin), meta={'asin': asin})
    ...
    def parse(self, response):
        ...
        for review in response.css('#cm_cr-review_list [data-hook="review"]'):
            ...
            review_date_text = review.css(
                '[data-hook="review-date"] ::text').get()
            ...
            item['StarsText'] = review.css(
                '[data-hook="review-star-rating"] ::text').get()
            item['Title'] = review.css(
                '[data-hook="review-title"] span ::text').get()
            item['ReviewDate'] = date_part
            item['ReviewedAt'] = place

            item['URL'] = response.url

            item['Review'] = review.xpath(
                'normalize-space(.//*[@data-hook="review-body"])').get()
